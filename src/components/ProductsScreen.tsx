    ...
    <SafeAreaView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      ...
      >
        ...
        <View style={styles.actions}>
          <View>
            <IconButton
              icon={() => <Ionicons name="cart" size={width * 0.06} color="#fff" />}
            ...
            {cartItemsCount > 0 && (
              <Badge
                size={height * 0.025}
                style={{ position: 'absolute', top: 2, right: 2, backgroundColor: theme.colors.error, color: '#fff' }}
              >
          ...
          {cartItemsCount > 0 && (
            <IconButton
              icon={() => <Ionicons name="trash-outline" size={width * 0.06} color="#fff" />}
          ...
          <IconButton
            icon={() => <Ionicons name="log-out-outline" size={width * 0.06} color={theme.colors.error} />}
            onPress={logout}
      ...
      {/* Searchbar */}
      <View style={{ paddingHorizontal: width * 0.03, marginBottom: height * 0.01 }}>
        <Searchbar
      ...
      >
        {loading && products.length === 0 ? (
          ...
          <FlatList
            ...
            renderItem={({ item }) => (
              <ProductCard product={item} onAdd={(ui) => add(toCartProduct(ui), 1)} />
            )}
            contentContainerStyle={{ paddingHorizontal: width * 0.03, paddingBottom: height * 0.15 }}
            showsVerticalScrollIndicator={false}
            onEndReached={onEndReached}
            onEndReachedThreshold={0.4}
            refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} />}
            ListFooterComponent={loadingMore ? <ActivityIndicator style={{ paddingVertical: 16 }} /> : null}
          />
        )}
      ...
      {cartItemsCount > 0 && (
        <FAB
          ...
          style={[
            styles.fab,
            { bottom: height * 0.03, right: width * 0.04, backgroundColor: theme.colors.success },
          ]}
          color="#fff"
          onPress={() => navigation.navigate('Cart')}
        />
...
const styles = StyleSheet.create({
  ...
  fab: { position: 'absolute', borderRadius: 30, elevation: 6 },
});
